...
![ReLINEの全体像。休眠検知 → きっかけ配信 → 会話再開 → 効果の可視化という利用フローと、主な機能、システムアーキテクチャ（LINEプラットフォーム → CatLineBot → 各種イベントハンドラ → ActiveJob → LineMessaging）、データと状態管理（MySQL 8.0の7テーブル）、技術スタック、テスト、セキュリティ、運用・可観測性をまとめた図](/readme-images/reline-overview.webp)

> ④の管理ダッシュボードに表示している数値・グループ名は画面イメージ用のサンプルです。

各項目の詳細は以下のセクションで説明します。
